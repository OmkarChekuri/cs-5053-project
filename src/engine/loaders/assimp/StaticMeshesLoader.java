...
public class StaticMeshesLoader {
    ...
    protected static void setMinAndMax(AIMesh aiMesh, Mesh mesh) {
        ...
        AIVector3D.Buffer aiVertices = aiMesh.mVertices();
        while (aiVertices != null && aiVertices.remaining() > 0) {
            AIVector3D aiVertex = aiVertices.get();
            max.x = Math.max(max.x, aiVertex.x());
            max.y = Math.max(max.y, aiVertex.y());
            max.z = Math.max(max.z, aiVertex.z());

            min.x = Math.min(min.x, aiVertex.x());
            min.y = Math.min(min.y, aiVertex.y());
            min.z = Math.min(min.z, aiVertex.z());
        }
